...
class InterpretabilityAnalyzer:
    ...
    def plot_feature_importance_analysis(self, save_path):
        ...
        ax1.set_title('Top 20 Feature Importance (Source Domain)', fontsize=14, fontweight='bold')
        ...
        # 3. 特征重要性分布
        ax3 = axes[1, 0]
        ax3.hist(feature_importance, bins=30, alpha=0.7, color='#A23B72', edgecolor='black')
        ax3.axvline(np.mean(feature_importance), color='red', linestyle='--', 
                   label=f'Mean: {np.mean(feature_importance):.3f}')
        ax3.axvline(np.median(feature_importance), color='orange', linestyle='--', 
    ...
    def plot_feature_correlation_analysis(self, save_path):
        ...
        if high_corr_pairs:
            ...
            top_pairs = high_corr_pairs[:10]  # 取前10对
        ...
        ax3.hist(corr_values, bins=50, alpha=0.7, color='#F18F01', edgecolor='black')
        ax3.axvline(np.mean(corr_values), color='red', linestyle='--', 
                   label=f'Mean: {np.mean(corr_values):.3f}')
        ax3.axvline(0, color='black', linestyle='-', alpha=0.5, label='Zero correlation')
    ...
    def plot_feature_distribution_analysis(self, save_path):
        ...
        for i, feature in enumerate(top_features):
            ...
            for j, label in enumerate(np.unique(self.y_source)):
                ...
                ax.hist(data, bins=20, alpha=0.6, label=label, density=True)
    ...
    def plot_feature_selection_analysis(self, save_path):
        ...
        ax2.set_title('Top 20 Features by Permutation Importance', fontsize=14, fontweight='bold')
        ax2.grid(True, alpha=0.3)
    ...
    def plot_transfer_process_visualization(self, save_path):
        ...
        ax4.hist(distances, bins=20, alpha=0.7, color='#F18F01', edgecolor='black')
    ...
    def plot_attention_visualization(self, save_path):
        ...
        ax1.set_title('Feature Attention Heatmap (Top 20)', fontsize=14, fontweight='bold')
        ...
        # 按特征类型分组
        feature_types = {
            'Time Domain': [f for f in self.feature_names if any(x in f.lower() for x in ['mean', 'std', 'rms', 'max', 'min', 'skewness', 'kurtosis'])],
            'Frequency Domain': [f for f in self.feature_names if any(x in f.lower() for x in ['spectral', 'psd', 'order'])],
            'Time-Frequency': [f for f in self.feature_names if any(x in f.lower() for x in ['envelope', 'vmd', 'wavelet'])],
            'Impact Features': [f for f in self.feature_names if any(x in f.lower() for x in ['impact', 'impulse', 'crest', 'shape'])]
        }
        ...
        bars = ax3.barh(range(len(top_15_features)), top_15_importance, 
                       color=plt.cm.viridis(np.linspace(0, 1, len(top_15_features))))
        ...
        # 4. 注意力稳定性分析
        ax4 = axes[1, 1]
        # 计算注意力的稳定性（基于特征重要性的一致性）
        # 这里使用简化的稳定性指标
        stability_scores = []
        for i, feature in enumerate(self.feature_names):
            # 计算该特征在不同样本上的重要性变化
            # 这里使用简化的方法
            stability = 1.0 / (1.0 + np.std(feature_importance))  # 简化的稳定性指标
            stability_scores.append(stability)
        
        # 绘制稳定性 vs 重要性
        ax4.scatter(feature_importance, stability_scores, alpha=0.7, s=60, c=range(len(feature_importance)), cmap='plasma')
...
def main():
    """主函数"""
    # 创建可解释性分析器
    analyzer = InterpretabilityAnalyzer('../02_特征提取/comprehensive_features.csv')
